...
def storeDocuments(containername, chunksize, overlap, endpointValue, apiValue):
    try:
        print(type(apiValue))
        print(type(endpointValue))
        search_client = SearchClient(
            endpoint=endpointValue, 
            index_name=containername, 
            credential=AzureKeyCredential(apiValue)
        )
        print('hello')
...
def moveToVectorStoreFunction(containername, domainname, versionid, chunksize, overlap, filename, endpointValue, apiValue):
    try:
        search_client = SearchClient(
            endpoint=endpointValue,
            index_name=containername,
...
def  createIndexFucntion(collection_name, endpoint, apiKey):
    try:
        client = SearchIndexClient(endpoint, AzureKeyCredential(apiKey))
...
def delete_index_function(collection_name, endpointValue, apiValue):
    client = SearchIndexClient(endpointValue, AzureKeyCredential(apiValue))
    try:
...
def delete_embeddings_function(blobName, collection_name, endpointValue, apiValue):
    search_client = SearchClient(endpointValue, collection_name, AzureKeyCredential(apiValue))
    try: 
        print(blobName)     
